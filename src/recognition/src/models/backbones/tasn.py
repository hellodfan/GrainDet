...
class Tasn(nn.Module):
    ...
    def __init__(self, pretrained, progress, num_classes=8, temperature=0.1, low_dim=128, proto_m=0.999):
        ...
        self.fc_cls  = nn.Linear(512 * 4, num_classes)

        self.fc_feat = nn.Linear(512 * 4, low_dim)
        self.l2norm = Normalize(2)

        self.register_buffer("prototypes", torch.zeros(num_classes, low_dim))
    ...
    def forward(self, x, targets):
        ...
        feats = self.pool_cls(conv_cls)
        feats = torch.flatten(feats, 1)
        out_cls = self.fc_cls(feats)
        out_structure, out_detail = torch.chunk(out_cls, 2)

        feats = self.l2norm(self.fc_feat(F.relu(feats)))
        prototypes = self.prototypes.clone().detach()
        logits_proto = torch.mm(feats, prototypes.t())/self.temperature
        proto_structure, proto_detail = torch.chunk(logits_proto, 2)       
        ...
        return out_att, out_structure, out_detail, proto_structure, proto_detail

def tasn_model(pretrained=False, progress=True, **kwargs):

    return Tasn(pretrained = pretrained, progress = progress, **kwargs)
